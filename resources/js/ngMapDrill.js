...
(function (root, factory) {
    if (typeof define === 'function' && define.amd) {
        define(['angular', 'echarts', 'china'], factory);
    } else if (typeof exports === 'object') {
        factory(require('angular'), require('echarts'), require('china'));
        module.exports = 'ngMapDrill';
    } else {
    ...
    angular
        .module('ngMapDrill', [])
        .directive('mapDrillChart', function () {
            return {
                restrict: 'EA',
                scope: {
                    data: '=',
                ...
                link: function ($scope, element) {
                    ...
                    var resizeMainContainer = function () {
                        element[0].style.width = container.width() - 80 + 'px';
                        element[0].style.height = container.height() - 80 + 'px';
                    };
                    resizeMainContainer();
                    ...
                    //34个省、市、自治区的名字地区配色映射数组
                    var areaColor = {'台湾': '#d499be', '河北': '#d498c1', '山西': '#80c679', '辽宁': '#fc8331', '吉林': '#80c678', '黑龙江': '#d49abd', '江苏': '#d49abd', '浙江': '#f6ca7b', '安徽': '#80c679', '福建': '#f88232', '江西': '#d49abd', '山东': '#f8ec6e', '河南': '#8aadd4', '湖北': '#f8ec6e', '湖南': '#80c679', '广东': '#f8ec6e', '海南': '#80ce79', '四川': '#f6ca7b', '贵州': '#8aadd4', '云南': '#f8ec6e', '陕西': '#d49abd', '甘肃': '#c5e2b0', '青海': '#8aadd4', '新疆': '#f6ca7b', '广西': '#d798bf', '内蒙古': '#f8ec6e', '宁夏': '#e8782d', '西藏': '#c5e2b0', '北京': '#f8e668', '天津': '#88d387', '上海': '#8aadd4', '重庆': '#f88231', '香港': '#f68534', '澳门': '#d49abd'};
                    ...
                    for (var key in provinces) {
                        (function (key) {
                            //注册34个省、市、自治区二级地区
                            $.getJSON('/resources/map/province/' + provinces[key] + '.json', function (data) {
                    ...
                    myChart.on('click', function (params) {
                        if (_.get(params, 'data.commonRegionId')) {
                            $scope.commonRegionId = _.get(params, 'data.commonRegionId');
                            $scope.currentRegionName = _.get(params, 'data.name');
                        }
                        if (params.name in provinces) {
                            if (_.get(params, 'data.commonRegionId')) {
                                $scope.navigationList.push({
                                    name: params.name,
                                    commonRegionId: _.get(params, 'data.commonRegionId')
                                });
                                $scope.level = params.name;
                            ...
                            if (special.indexOf(params.seriesName) < 0) {
                                if (_.get(params, 'data.commonRegionId')) {
                                    ...
                                    //显示县级地图
                                    $.getJSON('/resources/map/city/' + cityMap[params.name] + '.json', function (data) {
                    ...
                    var mapdata = [];
                    $scope.$watch('data', function (nData) {
                        if (nData) {
                            if ($scope.level === 'china') {
                                mapdata = _.map(nData, function (data) {
                                    return {
                                        name: data.REGION_ALIAS_NAME,
                                        value: data.INDEX_COUNT,
                                        commonRegionId: data.COMMON_REGION_ID,
                                        itemStyle: {
                                            color: areaColor[data.REGION_ALIAS_NAME]
                                        }
                                    }
                                });
                            } else {
                                mapdata = _.map(nData, function (data) {
                                    return {
                                        ...
                                        commonRegionId: data.COMMON_REGION_ID,
                                        itemStyle: {
                                            color: areaColor[data.PROVINCE_ALIAS_NAME]
                                        }
                                    }
                                });
                            }
                            setTimeout(function () {
                                renderMap($scope.level, mapdata);
                            }, 0);
                        }
                    }, true);
